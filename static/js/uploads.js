/* ==========================================================
   ...
   Supports:
     ...
     • SS selected-term view with General/root fallback
     • Year / class / term / subject filtering
     • Search
     • JSON preview
     • JSON delete
     • Push queue selection
     • DOCX one / multiple / drag-and-drop picker
========================================================== */
...
function flashMessage(text, type = "success") {
  ...
  window.__emisUploadFlashTimer = setTimeout(() => fm.classList.remove("show"), 3000);
}
...
(() => {
  ...
  window.EmisUploads = {
    ...
    normalizeSubject(value) {
      return String(value || "").toLowerCase().replaceAll("_", " ").replaceAll("-", " ").replace(/\s+/g, " ").trim();
    },

    isTermAwareClass(value) {
      return this.termAwareClasses.includes(this.normalizeClass(value));
    },

    isTermRequiredClass(value) {
      return this.termRequiredClasses.includes(this.normalizeClass(value));
    },

    isOptionalTermClass(value) {
      return this.optionalTermClasses.includes(this.normalizeClass(value));
    },
    ...
    getItemClass(item) {
      return this.normalizeClass(item?.class_category || item?.class_level || item?.class || "");
    },

    getItemTerm(item) {
      const cls = this.getItemClass(item);

      if (!this.isTermAwareClass(cls)) return null;
    ...
    getSubjectLabel(item) {
      ...
      const filename = String(item?.filename || "").replace(/\.json$/i, "").replace(/_(jss1|jss2|jss3|ss1|ss2|ss3)$/i, "").replaceAll("_", " ").replaceAll("-", " ").replace(/\s+/g, " ").trim();
    ...
    parseSelectionKey(key) {
      ...
      return { year, class_category: classCategory, term: rawTerm === "NONE" ? null : rawTerm, filename: filenameParts.join("|") };
    },

    getMeta(filename, year, classCategory = null, term = undefined) {
      const wantedClass = classCategory ? this.normalizeClass(classCategory) : null;
      const compareTerm = term !== undefined;
      const wantedTerm = term ? this.normalizeTerm(term) : null;

      return this.convertedItems.find((item) => {
        if (String(item.filename) !== String(filename) || String(item.year) !== String(year)) return false;
        if (wantedClass && this.getItemClass(item) !== wantedClass) return false;
        if (compareTerm && this.getItemTerm(item) !== wantedTerm) return false;
    ...
    getSelectedItems() {
      return [...this.selectedFiles].map((key) => {
        const selected = this.parseSelectionKey(key);

        return this.getMeta(selected.filename, selected.year, selected.class_category, selected.term) || selected;
      }).filter(Boolean);
    },

    getSelectedPushEntries() {
      return this.getSelectedItems().map((item) => ({ year: String(item.year || ""), filename: String(item.filename || ""), class_category: this.getItemClass(item), term: this.getItemTerm(item) }));
    },
...
updatePushCount() {
  ...
  /* Keep PDF quick-action visible even when "All" class is active */
  const termFilterBar = document.getElementById("termFilterBar");
  const termAware = this.isTermAwareClass(this.activeClass);

  if (termFilterBar) {
    const exportOnly = !termAware && count > 0;

    termFilterBar.classList.toggle("hidden", !termAware && count === 0);
    termFilterBar.classList.toggle("export-only", exportOnly);
  }

  document.dispatchEvent(new CustomEvent("emis:upload-selection-changed", { detail: { count } }));
},
    ...
    initOnce(container = document) {
      ...
      const previewEssayStatus = root.querySelector("#previewEssayStatusPill");
      const previewBody = root.querySelector("#latestPreviewBody");

      const jsonPreviewModal = root.querySelector("#jsonPreviewModal");
      const jsonPreviewTitle = root.querySelector("#jsonPreviewTitle");
      const jsonPreviewBody = root.querySelector("#jsonPreviewBody");
      const jsonPreviewObjectiveCount = root.querySelector("#jsonPreviewObjectiveCount");
      const jsonPreviewEssayCount = root.querySelector("#jsonPreviewEssayCount");
      const jsonPreviewEssayStatus = root.querySelector("#jsonPreviewEssayStatus");
      ...
      const setLoadingState = (loading) => {
        ...
        refreshBtn.innerHTML = loading ? `<span class="spinner"></span>` : `<i class="fa-solid fa-rotate"></i>`;
      };
      ...
      const resetPreview = () => {
        ...
        if (previewEssayStatus) previewEssayStatus.textContent = "—";
        if (previewBody) previewBody.innerHTML = `<p class="preview-placeholder">Select a file to preview.</p>`;

        if (jsonPreviewTitle) jsonPreviewTitle.textContent = "JSON Preview";
        if (jsonPreviewBody) jsonPreviewBody.textContent = "";
        if (jsonPreviewObjectiveCount) jsonPreviewObjectiveCount.textContent = "—";
        if (jsonPreviewEssayCount) jsonPreviewEssayCount.textContent = "—";
        if (jsonPreviewEssayStatus) jsonPreviewEssayStatus.textContent = "—";
      };
      ...
      const renderEmpty = (title, subtitle = "") => {
          ...
          <tr>
            <td colspan="9" class="empty">
              <div class="table-empty-state">
                <span class="table-empty-icon"><i class="fa-solid fa-folder-open"></i></span>
        ...
        if (libraryResultCount) libraryResultCount.textContent = "0 files";
      };


      /* ====================================================
         TERM UI / HYBRID SS FILTERING
      ==================================================== */
      ...
      const resetTermFilter = () => {
        ...
        termPills.forEach((pill) => {
          pill.classList.toggle("active", String(pill.dataset.term || "").toUpperCase() === "ALL");
        });
      };

    const updateTermVisibility = () => {
      const termAware = this.isTermAwareClass(this.activeClass);
      const optionalTerm = this.isOptionalTermClass(this.activeClass);
      const selectedCount = this.selectedFiles.size;
      const exportOnly = !termAware && selectedCount > 0;

      if (termFilterBar) {
        termFilterBar.classList.toggle("hidden", !termAware && selectedCount === 0);
        termFilterBar.classList.toggle("export-only", exportOnly);
      }

      const generalFilter = root.querySelector("#generalTermFilter");
      if (generalFilter) generalFilter.classList.toggle("hidden", !optionalTerm);

      if (!optionalTerm && this.activeTerm === "GENERAL") resetTermFilter();
      ...
      if (!termAware) resetTermFilter();
    };



      const updateActiveIndicators = () => {
        if (activeClassIndicator) activeClassIndicator.textContent = this.activeClass === "ALL" ? "All Classes" : this.activeClass;
        ...
        if (activeTermIndicator) {
          ...
          if (termAware) activeTermIndicator.textContent = this.activeTerm === "ALL" ? "All Terms" : this.termLabel(this.activeTerm);
        }
      };

      const filterRowsByActiveTerm = (rows) => {
        if (this.activeClass === "ALL" || !this.isTermAwareClass(this.activeClass) || this.activeTerm === "ALL") return rows;
        ...
        if (this.activeTerm === "GENERAL") {
          return rows.filter((item) => this.isOptionalTermClass(this.getItemClass(item)) && !this.getItemTerm(item));
        }


        const wantedTerm = this.normalizeTerm(this.activeTerm);

        if (!wantedTerm) return rows;
        ...
        if (this.isTermRequiredClass(this.activeClass)) {
          return rows.filter((item) => this.getItemTerm(item) === wantedTerm);
        }


        /* ----------------------------------------------------
           SS — TERM FILE FIRST, GENERAL FILE AS FALLBACK

           Example:

           SS1/FIRST/mathematics_ss1.json exists
               → show FIRST mathematics

           SS1/FIRST/poise_ss1.json missing
           SS1/poise_ss1.json exists
               → show General/root Poise
        ---------------------------------------------------- */
      ...
      const populateSubjectFilter = () => {
        ...
        let sourceRows = Array.isArray(this.convertedItems) ? [...this.convertedItems] : [];

        if (this.activeClass !== "ALL") sourceRows = sourceRows.filter((item) => this.getItemClass(item) === this.activeClass);

        sourceRows = filterRowsByActiveTerm(sourceRows);

        const subjects = [...new Set(sourceRows.map((item) => this.getSubjectLabel(item)).filter(Boolean))].sort((a, b) => a.localeCompare(b));

        subjectFilter.innerHTML = `<option value="ALL">All Subjects</option>${subjects.map((subject) => `<option value="${escapeHtml(subject)}">${escapeHtml(subject)}</option>`).join("")}`;

        const stillExists = [...subjectFilter.options].some((option) => option.value === current);
      ...
      const getFilteredRows = () => {
        let rows = Array.isArray(this.convertedItems) ? [...this.convertedItems] : [];


        /* Class */

        if (this.activeClass !== "ALL") rows = rows.filter((item) => this.getItemClass(item) === this.activeClass);


        /* Term / SS General fallback */

        rows = filterRowsByActiveTerm(rows);
        ...
        if (selectedSubject !== "ALL") {
          rows = rows.filter((item) => this.normalizeSubject(this.getSubjectLabel(item)) === this.normalizeSubject(selectedSubject));
        }


        /* Search */

        const query = String(searchBox?.value || "").toLowerCase().trim();
        ...
        if (query) {
          rows = rows.filter((item) => {
            ...
            const itemTerm = this.getItemTerm(item);
            const term = (itemTerm ? this.termLabel(itemTerm) : this.isOptionalTermClass(this.getItemClass(item)) ? "general" : "—").toLowerCase();

            return filename.includes(query) || subject.includes(query) || cls.includes(query) || term.includes(query);
          });
        }
      ...
      const syncCheckAll = () => {
        ...
        const selectedVisible = rows.filter((item) => this.selectedFiles.has(this.makeSelectionKey(item))).length;
      ...
      const renderTable = () => {
        ...
        tableBody.innerHTML = rows.map((item) => {
          ...
          const termDisplay = termRaw ? this.termLabel(termRaw) : this.isOptionalTermClass(classRaw) ? "General" : "—";
          const term = escapeHtml(termDisplay);
          ...
          /* ----------------------------------------------------
             ...
             GREEN:
             Objective > 0 OR Theory > 0

             RED:
             Objective = 0 AND Theory = 0
          ---------------------------------------------------- */

          const hasQuestions = objectiveCount > 0 || essayCount > 0;
          const questionRowClass = hasQuestions ? "question-ready-row" : "empty-json-row";
          ...
          const theoryLabel = `${essayCount} Theory`;
          const essayStatusLabel = hasEssay ? "Essay Included" : "No Essay";

          const statusRaw = String(item.status || "OK").toUpperCase();
          ...
          let statusLabel = hasQuestions ? "Has Questions" : "Empty JSON";
          let statusIcon = hasQuestions ? "fa-circle-check" : "fa-circle-xmark";

          if (statusRaw === "TERM_REQUIRED") {
            statusClass = "status-pill term-required";
            statusLabel = "Term Required";
            statusIcon = "fa-triangle-exclamation";

          } else if (statusRaw === "INVALID_JSON") {
            statusClass = "status-pill invalid-json";
            statusLabel = "Invalid JSON";
            statusIcon = "fa-circle-exclamation";

          } else if (hasQuestions) {
            statusClass = "status-pill question-ready";
            statusLabel = "Has Questions";
            statusIcon = "fa-circle-check";

          } else {
            statusClass = "status-pill empty-json-status";
            statusLabel = "Empty JSON";
          ...
          const termMeta = termRaw ? ` • ${escapeHtml(this.termLabel(termRaw))}` : this.isOptionalTermClass(classRaw) ? " • General" : "";
    ...
    return `
      <tr class="upload-row ${questionRowClass}" data-year="${year}" data-filename="${filename}" data-class="${classCategory}" data-term="${escapeHtml(termRaw || "")}" data-objective-count="${objectiveCount}" data-essay-count="${essayCount}" data-has-essay="${hasEssay ? "true" : "false"}" data-has-questions="${hasQuestions ? "true" : "false"}">
        ...
        <!-- CLASS -->
        <td><span class="summary-chip">${classCategory}</span></td>

        <!-- TERM -->
        <td class="term-column">
          ${
            termRaw || this.isOptionalTermClass(classRaw)
        ...
        <td class="actions-column">
          <div class="table-row-actions">
            ...
            <button class="mini-icon-button teacher-copy-btn" type="button" title="Teacher Copy" aria-label="Teacher copy ${filename}">
    ...
    updateStats();
      ...
      const loadYearFiles = async (year, refreshing = false) => {
        ...
        try {
          ...
          const response = await fetch(url, { method: "GET", cache: "no-store", headers: { Accept: "application/json", "Cache-Control": "no-cache", Pragma: "no-cache" } });
          ...
          console.log("[uploads] Library loaded:", { year, files: this.convertedItems.length });
      ...
      const applyPreviewToUI = (preview) => {
        if (!preview?.content) return;

        const { year, filename, classCategory, term, content, subject, version, objectiveCount, essayCount, hasEssay } = preview;

        const essayStatusLabel = hasEssay ? "Essay Included" : "No Essay";

        if (previewSubject) previewSubject.textContent = subject;
        if (previewVersion) previewVersion.textContent = version;
        if (previewQuestions) previewQuestions.textContent = essayCount ? `${objectiveCount} Objective • ${essayCount} Theory` : `${objectiveCount} Questions`;
        if (previewEssayStatus) previewEssayStatus.textContent = essayStatusLabel;
        ...
        if (previewBody) {
          previewBody.innerHTML = `
            <div class="preview-json-summary">
              ...
              <span>${escapeHtml(essayStatusLabel)}</span>
            </div>
        ...
        else if (this.isOptionalTermClass(classCategory)) titleParts.push("General");

        titleParts.push(filename);

        if (jsonPreviewTitle) jsonPreviewTitle.textContent = `Preview • ${titleParts.join(" • ")}`;
        if (jsonPreviewBody) jsonPreviewBody.textContent = JSON.stringify(content, null, 2);

        if (jsonPreviewObjectiveCount) jsonPreviewObjectiveCount.textContent = objectiveCount;
        if (jsonPreviewEssayCount) jsonPreviewEssayCount.textContent = essayCount;
        if (jsonPreviewEssayStatus) jsonPreviewEssayStatus.textContent = essayStatusLabel;
      };
      ...
      const previewFile = async (row) => {
        ...
        /* ----------------------------------------------------
           JSS root files are invalid.
           SS root files are valid General exams.
        ---------------------------------------------------- */

        if (this.isTermRequiredClass(classCategory) && !term) {
          throw new Error(`${classCategory} file has no term assigned.`);
        }
        ...
        const response = await fetch(previewUrl, { method: "GET", cache: "no-store", headers: { Accept: "application/json", "Cache-Control": "no-cache", Pragma: "no-cache" } });
        ...
        const objectiveCount = Array.isArray(content?.questions) ? content.questions.length : Number(meta?.objective_count ?? meta?.questions ?? 0);

        const essayCount = Array.isArray(content?.essay?.questions)
          ? content.essay.questions.length
          : Number(meta?.essay_count ?? meta?.essay_questions ?? 0);

        const explicitEssayFlag = meta?.has_essay ?? meta?.hasEssay ?? meta?.essay_present ?? meta?.theory_present ?? null;

        const hasEssay = Boolean(
          (content?.essay && typeof content.essay === "object" && !Array.isArray(content.essay) && essayCount > 0)
          || essayCount > 0
          || explicitEssayFlag === true
          || explicitEssayFlag === 1
          || String(explicitEssayFlag).toLowerCase() === "true"
        );

        const subjectLabel = this.getSubjectLabel(meta || content);
        const versionLabel = term ? this.termLabel(term) : this.isOptionalTermClass(classCategory) ? "General" : classCategory;
        ...
        applyPreviewToUI(window.__EMIS_UPLOAD_PREVIEW__);

        if (jsonPreviewModal) jsonPreviewModal.classList.remove("hidden");
      ...
      const deleteFile = async (row) => {
        ...
        const label = term
          ...
          : this.isOptionalTermClass(classCategory)
      ...
      termPills.forEach((pill) => {
        pill.addEventListener("click", () => {
          ...
          if (rawTerm === "GENERAL" && !this.isOptionalTermClass(this.activeClass)) return;


          termPills.forEach((item) => item.classList.remove("active"));
          pill.classList.add("active");

          this.activeTerm = rawTerm === "ALL" || rawTerm === "GENERAL"
            ? rawTerm
...
(() => {
  ...
  function initDocxPicker() {
    ...
    const chooseSingle = root.querySelector("#chooseSingle"), chooseMulti = root.querySelector("#chooseMulti");
    const inputSingle = root.querySelector("#uploadInputSingle"), inputMulti = root.querySelector("#uploadInputMulti");
    ...
    if (!chooseSingle || !chooseMulti || !inputSingle || !inputMulti || !dropZone) return;
    ...
    function isDocx(file) {
      return String(file?.name || "").toLowerCase().endsWith(".docx");
    }
    ...
    function addFiles(files, replace = false) {
      ...
      if (!incoming.length) {
        if (typeof flashMessage === "function") flashMessage("Please select DOCX files only.", "error");

        return;
      }

      if (replace) selectedDocxFiles = [];


      const existing = new Set(selectedDocxFiles.map(fileKey));
    ...
    function renderFiles() {
      ...
      if (noSelectedFiles) noSelectedFiles.classList.toggle("hidden", count > 0);
      ...
      fileList.innerHTML = selectedDocxFiles.map((file, index) => `
        <li class="selected-docx-item">
          ...
          <button type="button" class="remove-docx-file" data-index="${index}" title="Remove file">
    ...
    inputSingle.addEventListener("change", () => {
      if (inputSingle.files?.length) addFiles(inputSingle.files, true);
    });
    ...
    inputMulti.addEventListener("change", () => {
      if (inputMulti.files?.length) addFiles(inputMulti.files, false);
    });
    ...
    fileList?.addEventListener("click", (event) => {
      const button = event.target.closest(".remove-docx-file");
    ...
    clearUploads?.addEventListener("click", (event) => {
      ...
      if (typeof flashMessage === "function") flashMessage("Selected DOCX files cleared.", "success");
    });
  ...
  if (document.readyState === "complete" || document.readyState === "interactive") initDocxPicker();
  else document.addEventListener("DOMContentLoaded", initDocxPicker);


  new MutationObserver(initDocxPicker).observe(document.body, { childList: true, subtree: true });
