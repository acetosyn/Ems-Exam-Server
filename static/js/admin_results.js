...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function exportExcelCompatible() {
    ...
    const headerHtml = columns.map(([heading]) => `<th>${escapeHtml(heading)}</th>`).join("");

    const bodyHtml = records.map((row) => `
      <tr>${columns.map(([, getter]) => `<td>${escapeHtml(getter(row))}</td>`).join("")}</tr>
    `).join("");
    ...
    showToast("Excel-compatible file exported successfully.", "success");
  }
